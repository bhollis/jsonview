...
function register(onContentTypesChanged) {
  prefs.on(JSON_HTTP_ACCEPT_PREF_NAME, onPrefChange);
}

function unregister() {
  prefs.removeListener(JSON_HTTP_ACCEPT_PREF_NAME, onPrefChange);
...
function getJsonContentTypes() {
  var contentTypes = prefsService.get('extensions.jsonview@brh.numbera.com.sdk.baseURI').split(/[,\s]+/);
...
exports.getJsonContentTypes = getJsonContentTypes;
